 ...
 *                                currently queued in the edfvd queue it will
 ...
 *
 * edfvd_job_arrival(T)	- This is the catch all function when T enters
 *                                the system after either a suspension or at a
 *                                job release. It will queue T (which means it
 *                                is not safe to call edfvd_job_arrival(T) if
 *                                T is already queued) and then check whether a
 *                                preemption is necessary. If a preemption is
 *                                necessary it will update the linkage
 *                                accordingly and cause scheduled to be called
 *                                (either with an IPI or need_resched). It is
 *                                safe to call edfvd_job_arrival(T) if T's
...
DEFINE_PER_CPU(cpu_entry_t, edfvd_cpu_entries);

cpu_entry_t* edfvd_cpus[NR_CPUS];

/* the cpus queue themselves according to priority in here */
static struct bheap_node edfvd_heap_node[NR_CPUS];
static struct bheap      edfvd_cpu_heap;

/*EDFVD: storage for the tasks removed due to criticality change*/
static struct bheap      edfvd_ready_bin;
static struct bheap      edfvd_release_bin;
static rt_domain_t edfvd;
#define edfvd_lock (edfvd.ready_lock)
 ...
 * If the task been in edfvd bin for too long paramters will be stale, update them. 
 * */
static int edfvd_reset_params(struct bheap_node* node)
{
    struct task_struct* task = bheap2task(node);
   ...
   return(edfvd_check_criticality(node));
}
...
{
  clear_release_heap(&edfvd, &edfvd_release_bin, edfvd_check_criticality, edf_ready_order);
...
{
    int status = -1;
    bheap_iterate_clear(edfvd_check_criticality,edf_ready_order,&edfvd.ready_queue,&edfvd_ready_bin);   
    if(is_task_eligible(t)){
        replenish_task_for_mode(t);
...
static void lower_criticality(void)
{
   ...
   /*Iterate and move tasks of current criticality back to release queue.
     ...
     * */
    update_release_heap(&edfvd,&edfvd_release_bin,edf_ready_order,1); 
    if(current_criticality == 0){
        /*Clear all of the tasks from runqueue bin. No need to monitor that
         * */
        bheap_iterate_delete(edf_ready_order,&edfvd_ready_bin);
    }
}
 ...
 *                       order in the cpu queue. Caller must hold edfvd lock.
 */
static void update_cpu_position(cpu_entry_t *entry)
{
	if (likely(bheap_node_in_heap(entry->hn)))
		bheap_delete(cpu_lower_prio, &edfvd_cpu_heap, entry->hn);
	bheap_insert(cpu_lower_prio, &edfvd_cpu_heap, entry->hn);
}

/* caller must hold edfvd lock */
static cpu_entry_t* lowest_prio_cpu(void)
{
	struct bheap_node* hn;
	hn = bheap_peek(cpu_lower_prio, &edfvd_cpu_heap);
	return hn->value;
}
				      ...
				      cpu_entry_t *entry)
{
	...
	if (linked) {
		...
		if (on_cpu != NO_CPU) {
			sched = &per_cpu(edfvd_cpu_entries, on_cpu);
 ...
 *          where it was linked before. Must hold edfvd_lock.
 */
static noinline void unlink(struct task_struct* t)
{
	...
	if (t->rt_param.linked_on != NO_CPU) {
		/* unlink */
		entry = &per_cpu(edfvd_cpu_entries, t->rt_param.linked_on);
...
/* requeue - Put an unlinked task into edfvd domain.
 *           Caller must hold edfvd_lock.
 */
static noinline void requeue(struct task_struct* task)
{
	...
	if (is_early_releasing(task) || is_released(task, litmus_clock()))
		__add_ready(&edfvd, task);
	else {
		/* it has got to wait */
		add_release(&edfvd, task);
	}
}

#ifdef CONFIG_SCHED_CPU_AFFINITY
static cpu_entry_t* edfvd_get_nearest_available_cpu(cpu_entry_t *start)
{
	cpu_entry_t *affinity;

	get_nearest_available_cpu(affinity, start, edfvd_cpu_entries,
#ifdef CONFIG_RELEASE_MASTER
			edfvd.release_master,
#else
			NO_CPU,
#endif
            cpu_online_mask
...
static void check_for_preemptions(void)
{
	 ...
	 * idle. */
	local = this_cpu_ptr(&edfvd_cpu_entries);
	task  = __peek_ready(&edfvd);

	if (task && !local->linked
#ifdef CONFIG_RELEASE_MASTER
	    && likely(local->cpu != edfvd.release_master)
#endif
		) {
		task = __take_ready(&edfvd);
	...
	for (last = lowest_prio_cpu();
	     edf_preemption_needed(&edfvd, last->linked);
		...
		{
			cpu_entry_t *affinity =
					edfvd_get_nearest_available_cpu(
						&per_cpu(edfvd_cpu_entries, task_cpu(task)));
			if (affinity)
				last = affinity;
...
/* edfvd_job_arrival: task is either resumed or released */
static noinline void edfvd_job_arrival(struct task_struct* task)
{
	BUG_ON(!task);
...
static void edfvd_release_jobs(rt_domain_t* rt, struct bheap* tasks)
{
	unsigned long flags;
	...
	raw_spin_unlock_irqrestore(&edfvd_lock, flags);
}
...
{
	cpu_entry_t* entry = this_cpu_ptr(&edfvd_cpu_entries);
	 ...
	 */
	if (unlikely(edfvd.release_master == entry->cpu)) {
		sched_state_task_picked();
		return NULL;
	}
#endif

	raw_spin_lock(&edfvd_lock);
...
#ifdef WANT_ALL_SCHED_EVENTS
	TRACE_TASK(prev, "invoked edfvd_schedule.\n");
#endif
	 ...
	 */
	if (!entry->linked)
		link_task_to_cpu(__take_ready(&edfvd), entry);
    ...
    if(exists && !next)
        lower_criticality();
	raw_spin_unlock(&edfvd_lock);

#ifdef WANT_ALL_SCHED_EVENTS
	TRACE("edfvd_lock released, next=0x%p\n", next);
 ...
 */
static void edfvd_finish_switch(struct task_struct *prev)
{
	cpu_entry_t* 	entry = this_cpu_ptr(&edfvd_cpu_entries);
...
#ifdef CONFIG_RELEASE_MASTER
		if (entry->cpu != edfvd.release_master) {
#endif
			entry->scheduled = t;
...
static void edfvd_task_exit(struct task_struct * t)
{
	...
	if (tsk_rt(t)->scheduled_on != NO_CPU) {
		edfvd_cpus[tsk_rt(t)->scheduled_on]->scheduled = NULL;
		tsk_rt(t)->scheduled_on = NO_CPU;
	}
	raw_spin_unlock_irqrestore(&edfvd_lock, flags);
...
static void set_priority_inheritance(struct task_struct* t, struct task_struct* prio_inh)
{
	...
	if (linked_on != NO_CPU) {
		...
		bheap_delete(cpu_lower_prio, &edfvd_cpu_heap,
			    edfvd_cpus[linked_on]->hn);
		bheap_insert(cpu_lower_prio, &edfvd_cpu_heap,
			    edfvd_cpus[linked_on]->hn);
	} else {
		/* holder may be queued: first stop queue changes */
		raw_spin_lock(&edfvd.release_lock);
		if (is_queued(t)) {
		...
		raw_spin_unlock(&edfvd.release_lock);
...
static void edfvd_setup_domain_proc(void)
{
	...
	memset(&edfvd_domain_proc_info, sizeof(edfvd_domain_proc_info), 1);
	init_domain_proc_info(&edfvd_domain_proc_info, num_rt_cpus, 1);
	edfvd_domain_proc_info.num_cpus = num_rt_cpus;
	edfvd_domain_proc_info.num_domains = 1;

	edfvd_domain_proc_info.domain_to_cpus[0].id = 0;
	for (cpu = 0, i = 0; cpu < num_online_cpus(); ++cpu) {
		if (cpu == release_master)
			continue;
		map = &edfvd_domain_proc_info.cpu_to_domains[i];
		map->id = cpu;
		...
		cpumask_set_cpu(cpu,
			edfvd_domain_proc_info.domain_to_cpus[0].mask);
	}
}
...
static long edfvd_activate_plugin(void)
{
	...
	bheap_init(&edfvd_cpu_heap);
#ifdef CONFIG_RELEASE_MASTER
	edfvd.release_master = atomic_read(&release_master_cpu);
#endif

	for_each_online_cpu(cpu) {
		entry = &per_cpu(edfvd_cpu_entries, cpu);
		bheap_node_init(&entry->hn, entry);
		entry->linked    = NULL;
		entry->scheduled = NULL;
#ifdef CONFIG_RELEASE_MASTER
		if (cpu != edfvd.release_master) {
#endif
...
/*	Plugin object	*/
static struct sched_plugin edfvd_plugin __cacheline_aligned_in_smp = {
	.plugin_name		= "MIXCRIT",
	.finish_switch		= edfvd_finish_switch,
...
static int __init init_edfvd(void)
{
	...
	for (cpu = 0; cpu < NR_CPUS; cpu++)  {
		entry = &per_cpu(edfvd_cpu_entries, cpu);
		edfvd_cpus[cpu] = entry;
		entry->cpu 	 = cpu;
		entry->hn        = &edfvd_heap_node[cpu];
		bheap_node_init(&entry->hn, entry);
	}
	edf_domain_init(&edfvd, NULL, edfvd_release_jobs);
	return register_sched_plugin(&edfvd_plugin);
}
